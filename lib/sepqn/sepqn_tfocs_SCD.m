function [ x, odata, opts ] = sepqn_tfocs_SCD( objectiveF, affineF, dualproxF, mu, x0, z0, opts, contOpts )
...
if iscell(objectiveF)
    ...
    if length(mu)>1
      ...
      smoothF = @(varargin)sepqn_smooth_dual( objectiveF, mu, x0, varargin{:} );
    end
end

% ----------------------- Subfunctions ----------------------------------------

function [ prox, x ] = sepqn_smooth_dual( objectiveF, mu, x0, ATz )
% Adding 0 to ATz will destroy the sparsity
if (isscalar(x0) && x0 == 0) || numel(x0) == 0 || nnz(x0) == 0
